...
# ---------------------------------------------------------------------------
# Flip this to False to instantly disable preview postprocessing.
...
def _generate_raw_image_uint8(
    ...
    """
    Generate one silhouette and return it as a raw uint8 grayscale image.
    """
...
def _apply_graphic_silhouette_cv(image: np.ndarray) -> np.ndarray:
    ...
    # Light denoise before sharpening to avoid amplifying pixel noise.
    denoised = cv2.bilateralFilter(inverted, d=5, sigmaColor=25, sigmaSpace=25)
    # Unsharp mask to make garment boundaries clearer.
    blurred = cv2.GaussianBlur(denoised, (0, 0), 1.1)
    sharpened = cv2.addWeighted(denoised, 1.8, blurred, -0.8, 0)
    # Add a small edge boost so silhouettes read cleaner in the gallery.
...
def generate_image(
    ...
    """
    Generate one silhouette and return a larger preview for the Gradio gallery.
    Optional preview-only CV postprocess can be toggled via ENABLE_PREVIEW_CV_POSTPROCESS.
    """
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            complexity_slider = gr.Slider(
                ...
                info="Complexity of the generated silhouette, lower makes simpler shapes, higher makes chaotic shapes.",
            )
            morph_slider = gr.Slider(
                ...
                info="Interpolation factor for the style of the generated silhouette.",
            )
